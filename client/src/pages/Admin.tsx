...
export default function Admin() {
  ...
  async function addCertification() {
    if (!supabase) return;
    const { data, error } = await supabase.from("portfolio_certifications").insert({ title: "New certificate", caption: "", issuer: "", issue_date: "", credential_url: "", linkedin_url: "", image_url: "", sort_order: certs.length }).select().single();
  ...
  async function saveCertification(index: number) {
    ...
    const { error } = await supabase.from("portfolio_certifications").update({ title: item.title, caption: "", issuer: "", issue_date: "", credential_url: item.linkedin_url || "", linkedin_url: item.linkedin_url || "", image_url: "", updated_at: new Date().toISOString() }).eq("id", item.id);
