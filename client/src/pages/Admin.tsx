...
export default function Admin() {
  ...
  async function addCertification() {
    if (!supabase) return;
    const { data, error } = await supabase.from("portfolio_certifications").insert({ title: "New certification", caption: "Add a caption for this proof", issuer: "", issue_date: "", credential_url: "", image_url: "", sort_order: certs.length }).select().single();
  ...
  async function saveCertification(index: number) {
    ...
    const { error } = await supabase.from("portfolio_certifications").update({ title: item.title, caption: item.caption || item.title, issuer: item.issuer || "", issue_date: item.issue_date || "", credential_url: item.credential_url || "", image_url: item.image_url || "", updated_at: new Date().toISOString() }).eq("id", item.id);
