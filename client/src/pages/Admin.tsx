...
import { ArrowLeft, Eye, EyeOff, ImagePlus, KeyRound, LogIn, Save, ShieldCheck, Sparkles, Trash2, Upload } from "lucide-react";
...
export default function Admin() {
  ...
  async function signIn(event: React.FormEvent) {
    ...
    setSigningIn(true); setMessage(""); const { error } = await supabase.auth.signInWithPassword({ email, password }); setSigningIn(false); setMessage(error ? "That email or password did not work. Check your Supabase Auth user and try again." : "");
  }
  ...
  if (!session) return <div className="admin-shell login-shell"><div className="login-orbit login-orbit-one" /><div className="login-orbit login-orbit-two" /><main className="admin-login"><a href="/" className="admin-back"><ArrowLeft size={16} /> Back to portfolio</a><div className="login-card"><div className="login-brand"><span className="login-brand-mark"><Sparkles size={17} /></span><span>ARPIT / CONTENT STUDIO</span><span className="login-secure"><ShieldCheck size={14} /> SECURE</span></div><div className="login-heading"><p className="admin-kicker">PRIVATE WORKSPACE</p><h1>Welcome<br /><em>back.</em></h1><p>Sign in to curate your projects, screenshots, and live links.</p></div><form onSubmit={signIn}><label>Email address<div className="login-input"><KeyRound size={16} /><input type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" required /></div></label><label>Password<div className="login-input"><KeyRound size={16} /><input type={showPassword ? "text" : "password"} placeholder="Your secure password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" required /><button type="button" className="password-toggle" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></label><button className="admin-save login-submit" type="submit" disabled={signingIn}><LogIn size={16} /> {signingIn ? "Opening workspace…" : "Continue to dashboard"}</button></form><div className="login-footer"><button type="button" onClick={resetPassword}>Forgot password?</button><span>Supabase Auth protected</span></div>{message && <div className="login-error" role="alert">{message}</div>}</div><p className="login-footnote">Your project content stays protected. Only authenticated editors can change the public portfolio.</p></main></div>;
