...
export default function Admin() {
  ...
  async function loadProjects() {
    ...
    setItems((data || []).map((item: any) => ({ ...item, thumbnail_url: item.thumbnail_url || "", images: (item.portfolio_project_images || []).sort((a: any, b: any) => a.sort_order - b.sort_order).map((image: any) => image.image_url) })));
  }
  ...
  async function setThumbnail(index: number, image: string) {
    ...
    update(index, { thumbnail_url: image, images: [image, ...(items[index].images || []).filter((item) => item !== image)] });
    ...
    if (error) setMessage(error.message); else setMessage("Main thumbnail updated.");
  }
