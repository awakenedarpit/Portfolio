...
function Certifications() {
  ...
  return <section id="certifications" className="section section-light section-pad certifications-section"><div className="container"><SectionHeader index="08" title="Certifications" copy="A simple record of things learned, completed, and worth keeping." light /><div className="certification-list">{items.map((item, index) => <article className="certification-card simple-certification-card reveal" key={item.id || item.title}><span className="certification-mark">0{index + 1}</span>{item.image_url && <a className="certification-proof" href={item.image_url} target="_blank" rel="noreferrer"><img src={item.image_url} alt={item.caption || item.title} /><span>View photo</span></a>}<p className="certification-caption">{item.caption || item.title}</p></article>)}</div></div></section>;
}
