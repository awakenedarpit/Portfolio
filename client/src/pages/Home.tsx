...
function Certifications() {
  ...
  return <section id="certifications" className="section section-light section-pad certifications-section"><div className="container"><SectionHeader index="08" title="Certifications & proof of practice" copy="A growing record of the courses, programs, and milestones shaping the way I build." light /><div className="certification-list">{items.map((item, index) => <article className="certification-card reveal" key={item.id || item.title}><div className="certification-mark">0{index + 1}</div>{item.image_url && <a className="certification-proof" href={item.image_url} target="_blank" rel="noreferrer"><img src={item.image_url} alt={`${item.title} certificate proof`} /><span>View proof</span></a>}<div className="certification-copy"><p>{item.issuer}</p><h3>{item.title}</h3><span>{item.issue_date}</span></div>{item.credential_url ? <a className="certification-link" href={item.credential_url} target="_blank" rel="noreferrer">View credential <ArrowUpRight size={15} /></a> : <span className="certification-pending">Credential link coming soon</span>}</article>)}</div></div></section>;
}
